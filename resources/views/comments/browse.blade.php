...
{{--            @foreach($types as $type => $name)--}}
                <span>{{__('hotdesk.comment_ticket') .  __('hotdesk.comment_replied')}} {{ fa_number(App\Models\Comment::query()->where('reply_to', '<>' ,0)->count()) }}</span>
                <span>{{__('hotdesk.comment_ticket') . __('hotdesk.comment_waiting')}} {{ fa_number(App\Models\Comment::query()->where('reply_to', '=' ,0)->where('client_id' , '<>' , null)->count()) }}</span>
                <span>{{__('hotdesk.comment_ticket') .  __('hotdesk.comment_new')}} {{ fa_number(App\Models\Comment::query()->where('is_active', '=' ,0)->count()) }}</span>
{{--            @endforeach--}}
        </div>
                ...
                @foreach($dataTypeContent as $data)
                    <tr>
                        ...
                        <td class="no-sort no-click" id="bread-actions">
                            ...
                            <button class="openchat" data-id="{{$data->id}}"
                                    data-banned="{{App\Models\Banneduser::query()->where('user_id',$data->user_id)->exists() ? 'true' : 'false'}}"
                                    data-locked="{{$data->status == 'closed' ? 'true' : 'false'}}"
                                    data-name="{{$data->client ?  $data->client->first_name : ''}}"
                                    data-editurl="{{ $action->getRoute($dataType->name) }}"></button>
                        </td>
                        <td class="no-sort no-click"  id="bread-actions" style="cursor: pointer" onclick="sendVerify(this,{{ $data->id }})">
                            تایید
                        </td>
                    </tr>
                @endforeach
        ...
        <script>
            function sendVerify(element,id){
                $.post("{{ route('active_comment') }}",{id : id}).then(function(response){
                    console.log(response);
                    console.log($(element).parents().eq(0));
                    // var ParentEle = $(element).parents().eq(0).cells;
                    var ParentEle = $(element).closest("tr");
                    // $(ParentEle).eq(6).html("1");
                    location.reload();

                    console.log(ParentEle.length)
                })
            }
                    ...
                    <div class="list mCustomScrollbar" data-mcs-theme="dark">
                        @foreach(Auth::user()->messagetemplate()->orderby('created_at','desc')->get() as $messagetemplate)
                            <div class="item">
                                ...
                                <div class="action">
                                    <button type="button" class="deletetemplate"
                                            data-id="{{$messagetemplate->id}}"></button>
                                </div>
                            </div>
                        @endforeach
                    </div>
