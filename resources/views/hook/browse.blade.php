...
@section('page_header')
    <div class="action">
        <button class="btn btn-success install add">
            Add hook
        </button>
    </h1>
@stop
        ...
        <div class="modal-dialog">
            <form class="modal-content" action="{{ route('voyager.hooks') }}" id="install_form" method="POST">
                <div class="modal-header">
                    <button type="button" class="close" data-dismiss="modal" aria-label="Close"><span
                                aria-hidden="true">&times;</span></button>
                    <h4 class="modal-title">Install new hook.<i class="voyager-plus"></i></h4>
                </div>
...
@section('javascript')
    ...
    <script>
        $(document).ready(function () {
            $('#dataTable').DataTable({ "order": [] ,"language": {!! json_encode(__('datatable'), true) !!},"dom": '<"filters"f><"datatableserv"rt><"bottom"pli><"clear">',});
        });
        ...
        $('.action').on('click', '.install', function (e) {
