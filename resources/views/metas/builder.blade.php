...
@section('page_header')
    <h1 class="page-title">
        <i class="voyager-list"></i>متاساز ({{ $menu->name }})
        ...
        <div class="modal-dialog">
            <div class="modal-content">
                ...
                <div class="modal-footer">
                    <form action="{{ route('voyager.metas.item.destroy', ['meta' => $menu->id, 'id' => '__id']) }}"
                          id="delete_form"
                          method="POST">
                        {{ method_field("DELETE") }}
                        {{ csrf_field() }}
        ...
        <div class="modal-dialog">
            <div class="modal-content">
                ...
                <form action="" id="m_form" method="POST"
                      data-action-add="{{ route('voyager.metas.item.add', ['meta' => $menu->id]) }}"
                      data-action-update="{{ route('voyager.metas.item.update', ['meta' => $menu->id]) }}">

                    <input type="hidden" value="{{ $menu->id }}" name="meta_id"/>
                    {{ csrf_field() }}
                    <div class="modal-body">

                        <textarea class="form-control" id="m_color" name="content"
                                  placeholder="محتوای خود را وارد نمایید." rows="10"></textarea>

                        <br/>
                        <input class="form-control" id="m_color" name="link" placeholder="لینک مورد نظر خود را وارد نمایید."/>
    ...
    <script>
        $(document).ready(function () {
            ...
            $('.item_actions').on('click', '.delete', function (e) {
                ...
                $('#delete_form')[0].action = '{{ route('voyager.metas.item.destroy', ['meta' => $menu->id, 'id' => '__id']) }}'.replace('__id', id);
